...
LINK_CHECK_FAILURE_ABORT_RATIO = 0.9
...
def find_entries_to_remove(entries: Sequence[ApiEntry]) -> Tuple[List[ApiEntry], List[str], int]:
    ...
    return to_remove, report, link_failures
...
def main(readme_path: str, write_changes: bool) -> int:
    ...
    if entries and (link_failures / len(entries)) >= LINK_CHECK_FAILURE_ABORT_RATIO:
        print('Aborting automatic removals: link checks appear globally unavailable in this environment.')
        return 1
