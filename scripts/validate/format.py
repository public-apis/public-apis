...
def get_categories_content(
        contents: List[str]) -> Tuple[Categories, CategoriesLineNumber]:
...
def check_description(line_num: int, description: str) -> List[str]:
    ...
    if first_char.upper() != first_char:
        err_msg = error_message(
            line_num, 'first character of description is not capitalized')
        err_msgs.append(err_msg)

    last_char = description[-1]
    if last_char in punctuation:
        err_msg = error_message(
            line_num, f'description should not end with {last_char}')
        err_msgs.append(err_msg)
...
def check_auth(line_num: int, auth: str) -> List[str]:
    ...
    backtick = '`'
    if auth != 'No' and (not auth.startswith(backtick)
                         or not auth.endswith(backtick)):
        err_msg = error_message(line_num,
...
def check_entry(line_num: int, segments: List[str]) -> List[str]:
    ...
    err_msgs = [
        *title_err_msgs, *desc_err_msgs, *auth_err_msgs, *https_err_msgs,
        *cors_err_msgs
    ]
    ...
    for line_num, line_content in enumerate(lines):
        ...
        if line_content.startswith(anchor):
            ...
            if category_match:
                ...
                err_msg = error_message(
                    line_num, 'category header is not formatted correctly')
                err_msgs.append(err_msg)
        ...
        for segment in segments:
            ...
            if len(segment) - len(segment.lstrip()) != 1 or len(segment) - len(
                    segment.rstrip()) != 1:
                err_msg = error_message(
                    line_num,
